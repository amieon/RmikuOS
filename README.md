...
系统可以在 QEMU 上启动用户态 shell，并从真实的 virtio 块设备中加载 ext4 rootfs。当前系统已经支持用户程序加载、系统调用、进程与线程、VFS、多文件系统挂载（只读 ext4 + 可写 tmpfs + 可落盘的 FAT）、带读写双向访问的 virtio 块设备驱动、Unix 风格的 open flags 与文件权限、多级管道与重定向、功能完整的 shell，以及一套用于调度器实验的用户态 workload 与自适应调度控制器。用户程序可以用 **C 或 Rust** 编写，二者共享同一套系统调用 ABI。
...
构建产物进入 `user/build/<arch>/{bin,tests,programs}`，随后由 `user/mkfs_ext4.sh` 打包进 ext4 镜像；FAT 盘镜像由同一脚本单独生成（`mkfs.fat -F 16`）。生成的镜像位于：
