...
/// 返回 true = 本次消耗了一个 FIN(调用方需做状态转移)。
/// 教学简化:不考虑序号绕回;重叠段裁前缀;缓存超 OOO_CAP 才丢弃。
fn sr_input(t: &mut TcpSocket, seq: u32, payload: &[u8], fin: bool) -> bool {
    let mut seq = seq;
    let mut payload = payload;
    let mut fin_consumed = false;

    // 纯重复段:整体已确认,只触发重 ACK(由调用方负责)
    let end = seq.wrapping_add(payload.len() as u32 + fin as u32);
    if end <= t.rcv_nxt {
        return false;
    }
    // 头部重叠:裁掉已确认前缀(段边界通常对齐,这里是兜底)
    if seq < t.rcv_nxt {
        let skip = (t.rcv_nxt - seq) as usize;
    ...
    if seq == t.rcv_nxt {
        ...
        // 排水:缓存里能接上的段依次交付(这就是 SR 比 GBN 多做的事)
        while let Some(&k) = t.ooo.keys().next() {
            if k > t.rcv_nxt {
                break; // 还有洞,等重传
            }
            let (mut data, f) = t.ooo.remove(&k).unwrap();
            if k < t.rcv_nxt {
                // 部分重叠(洞被更大的段填了一部分):裁前缀
                let skip = (t.rcv_nxt - k) as usize;
                if skip >= data.len() {
                    if f { t.rcv_nxt = t.rcv_nxt.wrapping_add(1); fin_consumed = true; }
                    continue;
                }
                data = data.split_off(skip);
            }
            t.rcv_nxt = t.rcv_nxt.wrapping_add(data.len() as u32);
            t.rx_queue.push_back(data);
            if f {
                t.rcv_nxt = t.rcv_nxt.wrapping_add(1);
                fin_consumed = true;
                break; // FIN 之后不再有数据
        ...
        // 乱序到达:缓存(GBN 在这里是丢弃,这是唯一的本质区别)
        let buffered: usize = t.ooo.values().map(|(d, _)| d.len()).sum();
        if !t.ooo.contains_key(&seq) && buffered + payload.len() <= OOO_CAP {
            t.ooo.insert(seq, (payload.to_vec(), fin));
        }
        // 超出缓存上限才丢弃——此时只能等重传,行为退化回 GBN
    }
    fin_consumed
}
        ...
        if flags & RST != 0 {
            ...
            match t.state {
                ...
                | TcpState::CloseWait | TcpState::Closing | TcpState::LastAck => {
                    ...
                    if flags & SYN == 0 && (!payload.is_empty() || flags & FIN != 0) {
                        let fin_consumed = sr_input(t, seq, payload, flags & FIN != 0);
                        ack_now = true; // 数据/乱序/重复段都触发 ACK(重复 ACK 即对端 dup ACK 源)
                        if fin_consumed {
