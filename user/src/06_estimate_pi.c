...
static int rand_int(void) {
    return (int)(rand_u32() & RAND_MAX);
}

// 蒙特卡洛投点：在 [0,1) 正方形内随机投点，统计落在四分之一圆内的数量
static int monte_carlo_points(int num_points) {
    int inside = 0;
...
// ---------- 主程序 ----------
int main() {
    int num_children = 4;      // 默认子进程数
    int points_per_child = 10000; // 每个子进程投点数
    ...
    for (int i = 0; i < num_children; i++) {
        ...
        if (pid < 0) {
            ...
            int inside = monte_carlo_points(points_per_child);
            puts("[子进程 ");
            put_int(mypid);
            puts("] 圆内点数: ");
            put_int(inside);
