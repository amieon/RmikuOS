...
static void print_expected(int alpha) {
    if (alpha == 0) {
        puts("expected effective ratio: control:ai:logger ~= 6:2:1\n");
    } else if (alpha == 25) {
        puts("expected trend: between alpha=0 and alpha=50\n");
    } else if (alpha == 50) {
        puts("expected effective ratio: control:ai:logger ~= 3:3:1\n");
    } else if (alpha == 75) {
        puts("expected trend: between alpha=50 and alpha=100\n");
    } else if (alpha == 100) {
        puts("expected effective ratio: control:ai:logger ~= 3:9:2\n");
    }
}
...
static void print_stat_line(
    ...
    pos = append_str(buf, pos, "[alpha_stat] alpha=");
    ...
    pos = append_usize(buf, pos, (usize)base_tickets);

    pos = append_str(buf, pos, " stat_tickets=");
    pos = append_usize(buf, pos, (usize)st->tickets);
    ...
    pos = append_usize(buf, pos, total_work);
...
static int run_workload(int alpha, const char *name, int threads, int tickets) {
    if (threads <= 0 || threads > MAX_THREADS) {
        puts("[alpha_stat] FAIL: bad thread count\n");
        return 1;
    }

    if (set_my_tickets(tickets) < 0) {
        puts("[alpha_stat] FAIL: set_my_tickets\n");
        return 1;
    }

    if (get_my_tickets() != tickets) {
        puts("[alpha_stat] FAIL: get_my_tickets mismatch\n");
        return 1;
    }
    ...
    for (int i = 0; i < threads; i++) {
        ...
        if (tids[i] < 0) {
            puts("[alpha_stat] FAIL: thread_create\n");
            return 2;
        }
    }
    ...
    /*
     * 关键：在停止 worker 之前读取内核调度统计。
     * 此时 worker 线程还在 runnable 状态附近，ready/effective 更有参考价值。
     */
    struct sched_proc_stat st;

    if (get_process_sched_stat(getpid(), &st) < 0) {
        puts("[alpha_stat] FAIL: get_process_sched_stat\n");
        return 3;
    }
    ...
    for (int i = 0; i < threads; i++) {
        ...
        if (ret != tids[i] || code != 0) {
            puts("[alpha_stat] FAIL: thread_join\n");
            puts("i=");
            put_int(i);
            puts(" tid=");
            put_int(tids[i]);
    ...
    print_stat_line(alpha, name, threads, tickets, total, &st);

    return 0;
}

static int run_one_alpha(int alpha) {
    puts("\n[alpha_stat] run alpha=");
    put_int(alpha);
    puts("\n");

    print_expected(alpha);
...
int main(void) {
    puts("alpha_sched_stat_test start\n");
    puts("model: effective_tickets = base_tickets * ready_threads^alpha\n");
    puts("alpha values: 0, 25, 50, 75, 100 mean 0, 0.25, 0.5, 0.75, 1\n");
    puts("output includes both user total_work and kernel run_ticks\n");

    if (run_one_alpha(0) < 0) {
        return 1;
    }

    if (run_one_alpha(25) < 0) {
        return 1;
    }
    ...
    /*
     * 恢复默认 sqrt 策略，避免影响后续 shell 测试。
     */
    set_sched_alpha(50);

    puts("\nalpha_sched_stat_test PASS\n");
    puts("manual check:\n");
    puts("  alpha=0   control should dominate by tickets: about 6:2:1\n");
    puts("  alpha=50  control and ai should be close: about 3:3:1\n");
    puts("  alpha=100 ai should dominate by thread count: about 3:9:2\n");
    puts("  alpha=25/75 should fall between adjacent modes\n");
