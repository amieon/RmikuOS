...
ClassFile parse_class(const uint8_t* buf) {
    Reader r{buf};
    if (r.u4() != 0xCAFEBABE) { write(2,"bad magic\n",10); _exit(1); }
    ...
    for (int i = 1; i < n; i++) {
        ...
        switch (tag) {
        case 1: { int len=r.u2(); cf.cp[i].str.assign((const char*)r.p,len); r.p+=len; break; }
        case 7: cf.cp[i].u.class_name_idx = r.u2(); break;
        case 8: cf.cp[i].u.string_idx = r.u2(); break;
        ...
        case 9: case 10: case 11: cf.cp[i].u.ref.class_idx=r.u2(); cf.cp[i].u.ref.name_type_idx=r.u2(); break;
        case 12: cf.cp[i].u.nt.name_idx=r.u2(); cf.cp[i].u.nt.desc_idx=r.u2(); break;
        case 15: r.u1(); r.u2(); break;
        case 16: r.u2(); break;
        case 17: case 18: r.u2(); r.u2(); break;
        default: { char b[40]; int l=snprintf(b,40,"bad cp tag %d\n",tag); write(2,b,l); _exit(1); }
        }
    }
    cf.flags = r.u2();
    cf.this_class = r.u2();
    ...
    auto read_attrs = [&](Method* m) {
        for (int ac=r.u2(); ac>0; ac--) {
            ...
            if (m && cf.cp_utf8(ai)=="Code") {
                ...
                uint32_t cl=r.u4(); m->code.assign(r.p, r.p+cl); r.p+=cl;
                for (int e=r.u2(); e>0; e--) {
                    ExceptionEntry ex;
                    ex.start_pc=r.u2(); ex.end_pc=r.u2(); ex.handler_pc=r.u2(); ex.catch_type=r.u2();
