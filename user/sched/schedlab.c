 ...
 * schedlab.c —— 调度实验统一入口
 *
 * 六个实验,CLI 与原 tests/ 对齐:
 *   schedlab edf   [ctrl ai log] [total]              实验0 基线EDF(无退避,α=1)
...
int main(int argc, char **argv) {
    if (argc < 2) {
        printf("usage: schedlab <edf|mech|edge|aimd|dyn|adamw> ...\n");
        printf("  edf   [ctrl ai log] [total]          exp0: baseline EDF, no backoff\n");
        printf("  mech  <alpha> [t1 t2 t3] [total]     exp1: mechanism validation\n");
        printf("  edge  <alpha> [ctrl ai log] [total]  exp2: fixed-alpha trade-off\n");
        printf("  aimd  <alpha0> [ctrl ai log] [total] exp3: AIMD adaptive\n");
        printf("  dyn   <alpha0> [ctrl ai log] [total] exp4: dynamic load\n");
        printf("  adamw <alpha0> [ctrl ai log] [total] exp5: AdamW comparison\n");
        return 1;
    }
    ...
    if ((strcmp(mode, "edf") == 0)) {
        ...
        unsigned long total = (unsigned long)arg_or(argc, argv, 5, 36000);

        sl_add_jobs_parent("ctrl", 300, ctrl_t, /*period*/4, /*cpu*/2, /*burn*/400000);
        sl_add_spin("ai", 100, ai_t, 12000);
        sl_add_spin("log", 50, log_t, 12000);

        printf("# mode=edf alpha=1(fixed) ctrl=%d ai=%d log=%d total=%lu\n",
               ctrl_t, ai_t, log_t, total);

        sl_run(&(sl_cfg){
            .total_ticks  = total,
            .window_ticks = 100,
            .alpha0       = 1,       /* no backoff */
            .policy       = 0,       /* no adaptive policy */
            .policy_ud    = 0,
        });
        return 0;
    }
