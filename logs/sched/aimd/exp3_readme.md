...
- **AIMD 参数**：INC=1, BACKOFF=80%, safe_lateness=0, danger_lateness=50, cooldown=1

> AIMD 参数相对初版收紧：INC 5→1（精细逼近 edge，不跳过），safe 10→0（不允许任何迟到就算安全），danger 100→50（更早退避）。
...
**三个起点收敛到同一条稳态线**，α_steady 基本贴合 exp2 edge（medium/heavy 精准，light/medlo 偏差 ±5）。INC=1 精细逼近后不再像初版（INC=5）那样跳过 edge。
...
- `exp3_work_vs_miss.png` — ai 吞吐量 vs ctrl miss 散点（每配置）
...
### Bug 1：AIMD 全退到 0
**现象**：AIMD 从任何 α0 收敛到 0，miss 全 30%。
**根因**：`danger_lateness=25` 太敏感，in-parent ctrl 被主线程每 100tick 打断，late_delta 基线偏高，频繁触发退避。
**修复**：danger 50（精调后），safe 0。
...
### Bug 3：INC=5 跳过 edge
**现象**：AIMD α_steady 超过 exp2 edge（medium 34 vs edge 25）。
**根因**：INC=5 步长太大，probe up 时从 edge-5 直接跳到 edge+5，越过膝点。
**修复**：INC=1 精细逼近。
...
├── exp3_work_vs_miss.png             # throughput vs miss 散点
...
- **AIMD 参数**：INC=1, safe_lateness=0, danger_lateness=50（均非默认值）
